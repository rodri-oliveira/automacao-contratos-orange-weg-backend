...
async def rename_files():
    """
    Esta função processa:
    1. Arquivos QPE- com 6 números SEM letra (adiciona nome da cidade)
    2. Arquivos QPE- com 6 números COM letra (adiciona "FATURA-LOCAÇÃO")
    3. Arquivos SPB- com 6 números SEM letra (adiciona nome da cidade)
    4. Arquivos de TELECOM (BLU/POA/VIX/SPB/REC/BHO com 6 números + letra + 2 números) (adiciona "TELECOMUNICAÇÕES")
    
    Todas as outras regras estão COMENTADAS.
    """
    try:
        logger.info("=== INICIANDO PROCESSAMENTO DE ARQUIVOS (4 FUNÇÕES ATIVAS) ===")
        ...
        # Listar TODOS os arquivos da pasta ENTRADA (limite aumentado para 1000)
        ...
        for file in entrada_files:
            ...
            try:
                logger.info(f"Analisando arquivo {files_processed}/{len(entrada_files)}: {original_name}")
                
                # FUNÇÃO 1 ATIVA: QPE- com 6 números SEM letra
                if re.search(r"QPE-\d{6}(?![A-Za-z])", original_name):
                    logger.info(f"Arquivo identificado como QPE sem letra: {original_name}")
                        ...
                        if texto_pdf:
                            ...
                            if cidade_match:
                                ...
                                if success:
                                    ...
                                    renamed_files_qpe_sem_letra.append({
                                        "original": original_name,
                                        "new": new_name
                                    })
                                    logger.info(f"Arquivo renomeado com sucesso: {original_name} -> {new_name}")
                                ...
                                logger.warning(f"Não foi possível encontrar a cidade no texto do arquivo: {original_name}")
                        else:
                            logger.error(f"Não foi possível extrair texto do PDF: {original_name}")
                    else:
                        logger.error(f"Não foi possível baixar o arquivo: {original_name}")
                
                # FUNÇÃO 2 ATIVA: QPE- com 6 números COM uma letra no final
                elif re.search(r"QPE-\d{6}[A-Za-z]", original_name):
                    logger.info(f"Arquivo identificado como QPE com letra: {original_name}")
                    ...
                    if file_content:
                        ...
                        if success:
                            ...
                            renamed_files_qpe_com_letra.append({
                                "original": original_name,
                                "new": new_name
                            })
                            logger.info(f"Arquivo renomeado com sucesso: {original_name} -> {new_name}")
                ...
                # FUNÇÃO 3 ATIVA: SPB- com 6 números SEM letra no final
                elif re.search(r"SPB-\d{6}(?![A-Za-z])", original_name):
                    logger.info(f"Arquivo identificado como SPB sem letra: {original_name}")
                        ...
                        if texto_pdf:
                            ...
                            if cidade_match:
                                ...
                                if success:
                                    ...
                                    renamed_files_spb_sem_letra.append({
                                        "original": original_name,
                                        "new": new_name
                                    })
                                    logger.info(f"Arquivo renomeado com sucesso: {original_name} -> {new_name}")
                ...
                # FUNÇÃO 4 ATIVA: Arquivos de TELECOM (BLU/POA/VIX/SPB/REC/BHO + 6 números + letra + 2 números)
        ...
        return {
            "success": True,
            "message": f"{total_renamed} arquivos renomeados com sucesso",
            "detalhes": {
                "qpe_sem_letra": {
            ...
            "total_processado": files_processed,
